"""trainer_MGC.py: Model Training Class for MGCNets"""
...
class MGCTrainer(Trainer):
    ...
    def __init__(self, **args):
        super(MGCTrainer, self).__init__(**args)
    """

    def __init__(self, name, params, dataset, net):
        self.expr_name = name
        self.p = params
        ...
        self.writer = SummaryWriter(self.p.log_dir)
        self.w_iter = 0
        self.w_iter_val = 0
    ...
    def init_data_loader(self):
        ...
        for key in self.dataset.keys():
            ...
            self.data_loader[key] = DataLoader(self.dataset[key],
                                          ...
                                          sampler=sampler,
                                          **kwargs)
            self.n_batches[key] = len(self.dataset[key]) / self.p.batch_size
    ...
    def train_AE(self, validate=False):
        ...
        for ei in range(self.p.epoch):
            ...
            for di in tqdm(range(n_batches), desc="TRAIN"):
                ...
                # (32, 16, 1, 32, 32)
                # visualize
                self.visualize_AE_Training(ei, di, inputs, None, {'total_loss':total_loss, 'recon_loss':recon_loss,
        ...
        for ei in tqdm(range(num_epoch), desc=desc):
            if ei % self.update_interval == 0:
                ...
                if dname in [ Dataset.MNIST, Dataset.MICROGLIA_LABELED ]:
                    ...
                    self.report_accuracy(soft_labels, zs, gts, stage="Cluster_Training")
